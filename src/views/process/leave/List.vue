...
export default {
    ...
    return {
      // 表头
       columns: [
        ...
        {
          title: "请假类型",
          dataIndex: "leavetType_name",
          sorter: (a, b) => a.leavetType_name - b.leavetType_name ,
        },
        {
          title: "说明",
          dataIndex: "explain",
          sorter: (a, b) => a.explain - b.explain ,
        },
        ...
        return queryService(this.path.query_path, parameter).then(res => {
          let result = res.result
            ...
            res.result.data.forEach((value,index) => {
              ...
              value.qjsj = value.startDate+(value.startTime==1?"上午":"下午")+value.endDate+(value.endTime==1?"上午":"下午");
            });
          return res.result;
        });
      }
  ...
  created() {
    ...
    this.path.delete_path = prefix + "/leave/delete";
  },
  methods: {
