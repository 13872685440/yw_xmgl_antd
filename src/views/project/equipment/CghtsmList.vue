<template>
  <a-spin :spinning="spinning">
    <a-button v-if="!isview && state!='00100002'" type="primary" icon="plus" v-action:SYS_ADMINISTRATOR   @click="$refs.createCghtsm.add('')">新建</a-button>
    <a-table :columns="columns" :dataSource="data" :pagination="false" :loading="spinning" ref="table"></a-table>
    <create-cghtsm ref="createCghtsm" :keyValue="id" type="7" @ok="handleOk"/>
  </a-spin>
</template>
<script>
import createCghtsm from "./modules/CreateCghtsm";
...
export default {
  ...
  mounted() {
    ...
    this.path.find_path = prefix + "/equipment/find";
