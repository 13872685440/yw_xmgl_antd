<template>
  <div>
    <s-list v-if="currentTab === 'list'" :searchCode="searchCode" :XMSTATE="XMSTATE" @edit="edit" @search="search"/>
    <s-form v-if="currentTab === 'add'"  :formId="id" :type="type" :XMSTATE="XMSTATE" @edit="edit"/>
...
export default {
  ...
  created() {
    queryService(MODEL_SYSTEM_DICTIONARY, {typeCode: "XMSTATE"}).then((res) => {
      if (res.code === "400") {
