  ...
  <a-card :bordered="false">
    ...
    <s-table ref="table" size="default" :columns="columns" :data="loadData" :pageSize="20">
     <span slot="state" slot-scope="text, record">
        <template>
         <a title="采购合同扫描状态">
            <a-badge class="login-form-forgot" :style="productState(record.cghtsmState_code)"/>
          </a>
          <a title="付款与收票状态">
            <a-badge class="login-form-forgot" :style="productState(record.fkyspState_code)"/>
          </a>
          <a title="收货状态">
            <a-badge class="login-form-forgot" :style="productState(record.shState_code)"/>
          </a>
           <a title="退换货状态">
            <a-badge class="login-form-forgot" :style="productState(record.thhState_code)"/>
          </a>
        </template>
      </span>
...
export default {
  ...
  created() {
    ...
    this.path.delete_path = prefix + "/equipment/revoke";
  },
  methods: {
     productState(code) {
      var stype = "";
