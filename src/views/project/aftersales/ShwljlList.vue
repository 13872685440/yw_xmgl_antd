<template>
  <a-spin :spinning="spinning">
    <a-button v-if="!isview && state!='00100002'" type="primary" icon="plus" v-action:SYS_ADMINISTRATOR   @click="$refs.createShwljl.add('')">新建</a-button>
    <a-table :columns="columns" :dataSource="data" :pagination="false" :loading="spinning" ref="table"></a-table>
    <create-shwljl ref="createShwljl" :keyValue="id" type="11" @ok="handleOk" />
  </a-spin>
</template>
<script>
import createShwljl from "./modules/CreateShwljl";
...
export default {
  ...
  mounted() {
    ...
    this.path.find_path = prefix + "/aftersales/find";
