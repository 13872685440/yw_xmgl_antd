    ...
    <a-card :bordered="false" :bodyStyle="{ padding: '16px 0', height: '100%' }" :style="{ height: '100%' }" >
      <div class="account-settings-info-main" :class="device">
        ...
        <div class="account-settings-info-right">
          <s-base-view v-if="key === '1'" :base="base" :type="type" :formId="formId" />
          <s-hzhb-list v-if="key === '2'" :pid="formId" @changeForm="changeForm" :isview="true" />
          <s-jcbmyry-list v-if="key === '3'" :pid="formId" @changeForm="changeForm" :isview="true" />
        </div>
      </div>
    </a-card>
  </div>
...
export default {
  ...
  async created() {
    ...
    this.path.edit_path = prefix + "/customer/edit";
    this.selectedKeys = ["1"];
