  ...
  <a-card :bordered="false">
    ...
    <s-table ref="table" size="default" :columns="columns" :data="loadData" :pageSize="20">
      <span slot="action" slot-scope="text, record">
        ...
        <template>
          <a
            @click="handleEdit(record.id,'add')"
            title="编辑"
            v-if="!record.task_id && record.step===0"
          >
            <a-icon type="edit" style="color:#69aa46" />
          </a>
          <a
            @click="handleView(record.id,'add')"
            title="查看"
            v-if="record.task_id || record.step!=0"
          >
            <a-icon type="edit" style="color:#69aa46" />
          ...
          <a
            @click="$refs.simage.add({processInstanceId:record.processId})"
            v-if="record.task_id  || record.step!=0"
            title="查看流程图"
          >
          ...
          <a @click="handleDelete(record.id)" v-if="!record.task_id && record.step===0" title="删除">
            <a-divider type="vertical" />
